...
namespace tgw
{
    ...
    namespace
    {
        std::string ToString(beast::string_value value)
        {
        ...
        void ParseTarget(const std::string& target,std::string* path,std::string* query)
        {
            ...
            *quary = target.substr(pos+1);
        }

        HttpRequest ConvertBeastRequest(cosnt http::request<http::string_body>&req,const std::string* client_ip,const std::string& request_id)
        {
            HttpRequest tgw_req;
            ...
            {
                tgw_req.headers[ToString(filed.name_string())] = ToString(filed.values());
            }
    ...
    void HttpServer::Start()
    {
        ...
        while(true)
        {
            ...
            if(ec)
            {
                TGW_ERROR("accpet connection failed:{}",ec.message());
                continue;
            }
    ...
    void HttpServer::HandleSession(std::shared_ptyr<void> socket_holder)
    {
        ...
        try
        {
            ...
            auto remote_endpoint = socket->remote_endpointer(remote_ec);
            if(!remote_ec)
            {
                client_ip = remote_endpointer.address().to_string();
            }
            ...
            res.set(http::field::content_type,tgw_rsp.content_ytoe);
            res.set("S-Requesr-Id",request_id);
    ...
    {
        auto now = std::chrono::steady_clock::now().time_since_epoch().const();
